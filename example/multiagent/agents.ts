...
export const getAgents = (repository: string) => {
  ...
  const codeAnalyst = new Agent({
    persona: {
      role: 'Code Analyst',
      background: 'You specialize in node.js applications that use postgres as its database ',
      goal: dedent`Please, read through the database schema of the ${repository} application. Describe the schema and provide a list of tables and their relationships. 
    ...
    Do not depend on the repositry structure alone, read the code for better understanding. Repository name is "${repository}" do not read any other repository.`,
  })
  ...
  const aiEngineer = new Agent({
    persona: {
      ...
      goal: dedent`You are given a response from the Code Analyst. Validate the response and provide a list of improvements that can be made to the response.
    ...
    Do not suggest changes to the database schema, only to the documentation.
    You can read the code for better understanding. Repository name is "${repository}" do not read any other repository.`,
  })
  ...
  const seniorDatabaseArchitect = new Agent({
    persona: {
      ...
      You are experienced in database design and architecture. You are also proficient in the database schema and relationships documentation.`,
    },
    tools,
    llm,
    context: [codeAnalyst, aiEngineer],
    task: dedent`Work on improving the response from the Code Analyst. Make sure to follow the instructions from the AI Engineer. This relates to the ${repository} application.
    Use as many tools as needed. Do not depend on the repositry structure alone, read the code for better understanding. Repository name is "${repository}" do not read any other repository.`,
  })
