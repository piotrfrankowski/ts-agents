...
export class OllamaConnector implements LLMConnector {
  ...
  public async chat(messages: Message[]): Promise<Message> {
    ...
    do {
      ...
      const parts: OllamaResponse[] = data.filter(p => !!p).map(p => typeof p === 'string' ? JSON.parse(p) : p);
      ...
      for (const part of parts) {
        ...
        if (part.message.tool_calls) {
          const toolCallHash = createHash('md5').update(JSON.stringify(part.message.tool_calls)).digest('hex');
