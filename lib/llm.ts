...
export class LLM {
  ...
  constructor({ model, connector }: { model: string, connector: LLMConnector | 'ollama' | 'openai' }) {
    if (connector === 'ollama') {
  ...
  async execute(messages: Message[], agent: Agent, isGraph = false): Promise<{ final: string, withThoughts: string }> {
    let isDone = false;
    const finalResponse = {
      withThoughts: '',
      final: '',
    ...
    do {
      ...
      if (!message.tool_calls || message.tool_calls.length === 0) {
        if (message.content.includes('</final>') || message.content.includes('<final>') || isGraph) {
          ...
          messages.push({
            role: 'user',
            content: 'Please summarize your work and provide a final response with </final> tag.',
          })
        ...
        messages.push({
          role: 'user',
          content: 'Continue your work. Use the file reading tool if needed. Mark your final response with </final> tag if you no longer need to use the tools.',
        })
      ...
      for (const tool of message.tool_calls) {
        const id = tool.id || createHash('md5').update(JSON.stringify(tool.function)).digest('hex');
        if (toolCallsMap[id]) {
          continue;
        }

        const functionToCall = this.tools.find(t => t.name === tool.function.name)
        if (!functionToCall) {
          logger.warn(`Agent ${agent.name}: Tool not found`, tool.function.name);
          messages.push({
            role: 'tool',
            tool_call_id: id,
        ...
        const functionResponse = (await functionToCall.call(args)) || 'No response from the tool';
  ...
  private cleanupResponse(response: string): { final: string, think: string } {
    if (response.includes('<final>') && response.includes('</final>')) {
      ...
      return { final: response.slice(start, end), think: finalPos ?  response.slice(0, finalPos) : '' };
    }

    if (response.includes('</think>')) {
      const [think, final] = response.split('</think>');
      return { final: final?.replaceAll('</final>', '') || '', think: think?.replaceAll('</think>', '') || '' };
    }

    return { final: response.replaceAll('</think>', '').replaceAll('</final>', '').replaceAll('<final>', ''), think: '' };
  }

  private debugResponses(agent: Agent, message: Message): void {
    if (process.env['LOG_LEVEL'] === 'DEBUG') {
      appendFileSync(`out/${agent.runName}/debug.json`, JSON.stringify({ time: Date.now(), agent: agent.name, message }, null, 2) + ',\n')
    }
  }
}
